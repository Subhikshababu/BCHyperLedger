...
// This method checks if the given train exists and executes callback depending on success / failure
async function trainExists(trainID, success, failure){
    channel.queryByChaincode({
            chaincodeId: 'fabtrain',
            fcn: 'queryTrain',
            args: [trainID]
    ...
    Promise.all(promises).then((results) => {
        ...
        if(results && results[1] && results[1].event_status === 'VALID') {
            ...
            if (Object.keys(args).length === 2) {
                socket.emit('RESPONSE',{type: 'INFO' , payload:[{Key:`${args[0]} successfully changed to ${args[1]}!` ,Msg:'Successfully committed change to the ledger'}] })
            } else {
                socket.emit('RESPONSE',{type: 'INFO' , payload:[{Key:`Successfully created ${args[0]}!` ,Msg:'Successfully committed change to the ledger'}] })
            }
        } else {
        ...
        switch (req.action)
        {
            ...
            case "QUERYALL":
                ...
                query(
                    ...
                    break;
            case "CHANGE":
                socket.emit('RESPONSE', {type: 'START', payload: `Request for CHANGE for ${req.data.ID} to ${req.data.newStatus} received` });
                trainExists(req.data.ID, 
                            () => {invoke(
                                {
                                    chaincodeId: 'fabtrain',
                                    fcn: 'changeTrainStatus',
                                    args: [req.data.ID , req.data.newStatus],
                                    chainId: 'mychannel'
                                }
                            , socket)},
                            () => {
                    ...
                    () => {invoke(
                        {
                            chaincodeId: 'fabtrain',
                            fcn: 'createTrain',
                            args: [req.data.ID, req.data.fname, req.data.gender, req.data.place, req.data.class, req.data.status],
                            chainId: 'mychannel',
                        }
                    , socket)});
                    break;
